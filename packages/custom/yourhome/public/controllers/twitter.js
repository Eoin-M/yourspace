...
/* jshint -W098 */
angular.module('mean.yourhome').controller('TwitterController', ['$scope', '$rootScope', '$http', 'MeanUser', 'Global', 'Yourhome',
  function($scope, $rootScope, $http, MeanUser, Global, Yourhome) {
    $scope.global = Global;
	
	$scope.twitterLoggedin = 'Log in with';
	...
	if(MeanUser && MeanUser.user){
		if(MeanUser.user.twitter) {
			$scope.twitterLoggedin = undefined;
			$scope.openTwitterStream();
		}
		else if(MeanUser.user) $scope.twitterLoggedin = 'Link';
	}
